...
TYPE getMinHeap(DynArr *heap)
{
	...
	temp = getDynArr(heap, _minIdx(heap, 0, heap->size - 1));
...
void addHeap(DynArr *heap, TYPE node)
{
    ...
    while (pos > 0) {
        ...
        if (compare(heap->data[parent], heap->data[pos]) > 0) {
            /* TYPE temp = heap->data[parent];
...
int _minIdx(struct DynArr *da, int i, int j)
{
	...
	for (index = i; index < j; index++) {
		if (LT(da->data[index].priority, min_priority)) {
			min_priority = da->data[index].priority;
			min_index = index;
		}
	}
...
void sortHeap(DynArr *heap) {
	...
	for (i = 0; i < heap->size - 1; i++) {
        ...
        for (j = 0; j < heap->size - i - 1; j++) {
            if (heap->data[j].priority > heap->data[j + 1].priority) {
